...
class Emd_List_Table extends WP_List_Table {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$columns = array(
					...
					'name' => esc_html__( 'Name', 'youtube-showcase' ),
					'label' => esc_html__( 'Label', 'youtube-showcase' ),
					'entity' => esc_html__( 'Entity', 'youtube-showcase' ),
					'ctype' => esc_html__( 'Type', 'youtube-showcase' ),
					'dtype' => esc_html__( 'Display Type', 'youtube-showcase' ),
					'created'   => esc_html__( 'Updated', 'youtube-showcase' ),
				);
			}
			else {
				$columns = array(
					'cb'        => '<input type="checkbox" />',
					'name' => esc_html__( 'Name', 'youtube-showcase' ),
					'type' => esc_html__( 'Type', 'youtube-showcase' ),
					'shortcode' => esc_html__( 'Shortcode', 'youtube-showcase' ),
					'created'   => esc_html__( 'Created', 'youtube-showcase' ),
				);
				if($this->type == 'form'){
					$columns['modified'] = esc_html__( 'Modified', 'youtube-showcase' );
				}
			}
		}
		else {
	...
	public function column_default( $rec, $column_name ) {
		switch ( $column_name ) {
			...
			case 'ctype':
				if($rec['ctype'] == 'attr'){
					$value = __('Attribute','youtube-showcase');
				}
				elseif($rec['ctype'] == 'tax'){
					$value = __('Taxonomy','youtube-showcase');
				}
				break;
			case 'dtype':
				$value = $rec['dtype'];
			...
			case 'shortcode':
				$value = '<p id="shortcode_' . $rec['id'] . '">' . stripslashes($rec['shortcode']) . '</p>
					<a class="emd-copy-clipb button button-primary" style="padding:1px 6px;" data-clipboard-target="#shortcode_' . $rec['id'] . '">' . __('Copy','youtube-showcase') . '</a>';
				break;
			case 'created':
	...
	public function get_bulk_actions() {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$actions = array(
					'delete' => esc_html__( 'Delete Field', 'youtube-showcase' ),
					'delete_data' => esc_html__( 'Delete Field & Data', 'youtube-showcase' ),
				);
			}
			elseif($this->type == 'form'){
				$actions = array(
					'delete' => esc_html__( 'Delete', 'youtube-showcase' ),
					'export' => esc_html__( 'Export', 'youtube-showcase' ),
				);
			}
			else {
				$actions = array(
			...
			if (wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'bulk-' . $this->type . 's')){
				...
				<div class="notice updated">
					<p>
						<?php
						if ( count( $ids ) === 1 ) {
							esc_html_e( 'Record was successfully deleted.', 'youtube-showcase' );
						} else {
							esc_html_e( 'Records were successfully deleted.', 'youtube-showcase' );
						}
						?>
					</p>
				</div>
				...
				<div class="notice updated">
					<p>
						<?php esc_html_e( 'Security check failed. Please try again.', 'youtube-showcase' ); ?>
					</p>
				</div>
				<?php
			}
	...
	/**
		...
		if($this->type == 'form'){
			$name = sprintf(
				...
				esc_html__('Edit Form', 'youtube-showcase'),
				$name
			);
			...
			$row_actions['edit_settings'] = sprintf(
				...
				esc_html__('Edit Settings', 'youtube-showcase'),
				esc_html__('Edit Settings', 'youtube-showcase')
			);
			$row_actions['edit'] = sprintf(
				...
				esc_html__('Edit Layout', 'youtube-showcase'),
				esc_html__('Edit Layout', 'youtube-showcase')
			);
		...
		if($this->type == 'cfield'){
			$name = sprintf(
				...
				esc_html__('Edit', 'youtube-showcase'),
				$name
			);
		}
		return $name;
